...
const atividade = {
    nome: "Atividade",
    data: adicionarDias(dataAtual, 0),
    finalizada: true
}
...
const criarDiasSelecao = () => {
    const dias = Array.from({length: 7}, (_, i) => adicionarDias(dataAtual, i).toISOString().split('T')[0])
...
function showPosition(position) {
    ...
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}`)
        ...
        .then(data => {
            const address = data.address.city + ", " + data.address.state;
